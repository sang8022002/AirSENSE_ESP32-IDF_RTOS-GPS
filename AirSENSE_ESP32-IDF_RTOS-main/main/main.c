...
#include "../../component/nmea_parser/nmea_parser.h"
...
TaskHandle_t mqttPublishGpsTask_handle = NULL;
...
SemaphoreHandle_t sentGpsToMQTT_semaphore = NULL;
...
const char *formatDataGpsString = "{\n\t\"station_id\":\"%x%x%x%x\",\n\t\"Latitude\":%.5f,\n\t\"Longitude\":%.5f,\n\t\"Altitude\":%.2f,\n\t\"Speed\":%f\n}";
...
static esp_err_t WiFi_eventHandler(void *argument, system_event_t *event)
{
    ...
    case SYSTEM_EVENT_STA_GOT_IP:
        ...
        else
        {
            ...
            if (eTaskGetState(mqttPublishGpsTask_handle) == eSuspended)
            {
                vTaskResume(mqttPublishGpsTask_handle);
                ESP_LOGI(__func__, "Resume task mqttPublishGPSTask.");
            }
...
struct dataGPS_st GPS_lastTime;
#define TIME_ZONE (+8)   //Beijing Time
#define YEAR_BASE (2000) //date in GPS starts from 2000
static void gps_event_handler(void *event_handler_arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    gps_t *gps = NULL;
    struct dataGPS_st dataGPSTemp;
    sentGpsToMQTT_semaphore = xSemaphoreCreateMutex();
    switch (event_id)
    {
    case GPS_UPDATE:
        if(ds3231_isNewDay(&ds3231_device))
        {
            if (xSemaphoreTake(sentGpsToMQTT_semaphore, portMAX_DELAY) == pdPASS)
            {
                gps = (gps_t *)event_data;
                /* print information parsed from GPS statements */
                ESP_LOGI(__func__, "%d/%d/%d %d:%d:%d => \r\n"
                        "\t\t\t\t\t\tlatitude   = %.05f°N\r\n"
                        "\t\t\t\t\t\tlongitude = %.05f°E\r\n"
                        "\t\t\t\t\t\taltitude   = %.02fm\r\n"
                        "\t\t\t\t\t\tspeed      = %fm/s",
                ...
                dataGPSTemp.latitude = gps->latitude;
                dataGPSTemp.longitude = gps->longitude;
                dataGPSTemp.altitude = gps->altitude;
                dataGPSTemp.speed = gps->speed;
                
                if (xQueueSendToBack(dataGpsToMQTT_queue, (void *)&dataGPSTemp, WAIT_10_TICK * 5) != pdPASS)
                {
                    ESP_LOGE(__func__, "Failed to post the data GPS to dataGpsToMQTT_queue.");
                }
                else
                {
                    ESP_LOGI(__func__, "Success to post the data GPS to dataGpsToMQTT_queue.");
                }
                xSemaphoreGive(sentGpsToMQTT_semaphore);
            }
        }
...
void mqttPublishGps_task()
{
    sentGpsToMQTT_semaphore = xSemaphoreCreateMutex();

    for (;;)
    {
        struct dataGPS_st dataGPSReceiveFromQueue;
        if (statusDevice.mqttClient == CONNECTED)
        {
            if (uxQueueMessagesWaiting(dataGpsToMQTT_queue) != 0)
            {
                if (xQueueReceive(dataGpsToMQTT_queue, (void *)&dataGPSReceiveFromQueue, portMAX_DELAY) == pdPASS)
                {
                    ESP_LOGI(__func__, "Receiving data from queue successfully.");
                    if (xSemaphoreTake(sentGpsToMQTT_semaphore, portMAX_DELAY) == pdTRUE)
                    {
                        esp_err_t error = 0;
                        WORD_ALIGNED_ATTR char mqttMessage[256];
                        sprintf(mqttMessage, formatDataGpsString, MAC_address[0],
                                MAC_address[1],
                                MAC_address[2],
                                MAC_address[3],
                                dataGPSReceiveFromQueue.latitude,
                                dataGPSReceiveFromQueue.longitude,
                                dataGPSReceiveFromQueue.altitude,
                                dataGPSReceiveFromQueue.speed);

                        error = esp_mqtt_client_publish(mqttClient_handle, (const char *)mqttTopic, mqttMessage, 0, 0, 0);
                        xSemaphoreGive(sentGpsToMQTT_semaphore);
                        if (error == ESP_FAIL)
                        {
                            ESP_LOGE(__func__, "MQTT client publish message GPS failed ¯\\_(ツ)_/¯...");
                        }
                        else
                        {
                            ESP_LOGI(__func__, "MQTT client publish message GPS success (^人^).");
                        }
                    }
...
static void mqtt_app_start(void)
{
    ...
    xTaskCreate(mqttPublishGps_task, "MQTT Publish GPS", (1024 * 16), NULL, (UBaseType_t)10, &mqttPublishGpsTask_handle);
}
...
// Initialize RTC module
    ...
    dataGpsToMQTT_queue = xQueueCreate(30, sizeof(struct dataGPS_st));
    ...
    dataSensorIntermediate_queue = xQueueCreate(10, sizeof(struct dataSensor_st));
